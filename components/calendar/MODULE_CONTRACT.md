...
Подмодуль отвечает за preview внешних событий, недельную календарную сетку и UX импорта события в planner, включая подсветку importable событий, per-event dismiss и массовый импорт недели.

## Ответственность
- `calendar-screen.tsx` собирает route-level календарный экран, week navigation и bulk import trigger под grid.
- `calendar-week-grid.tsx` рисует недельную сетку, all-day row, timed events, event details modal и importable highlight/dismiss UX.
